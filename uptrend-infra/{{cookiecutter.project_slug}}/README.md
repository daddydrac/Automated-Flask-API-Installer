...
you can copy that URL and modify the "host" to be `localhost` or the domain you are using for development (e.g. `local.dockertoolbox.uptrend-data-science.com`), in the case above, it would be, e.g.:
...
As this is a common case, the domain `local.dockertoolbox.uptrend-data-science.com` points to that (private) IP, just to help with development (actually `dockertoolbox.uptrend-data-science.com` and all its subdomains point to that IP). That way, you can start the stack in Docker Toolbox, and use that domain for development. You will be able to open that URL in Chrome and it will communicate with your local Docker Toolbox directly as if it was a cloud server, including CORS (Cross Origin Resource Sharing).

If you used the default CORS enabled domains while generating the project, `local.dockertoolbox.uptrend-data-science.com` was configured to be allowed. If you didn't, you will need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `local.dockertoolbox.uptrend-data-science.com`.

After performing those steps you should be able to open: http://local.dockertoolbox.uptrend-data-science.com and it will be server by your stack in your Docker Toolbox virtual machine.
...
In that case, you have two options: you could use the instructions to modify your system `hosts` file with the instructions below in **Development with a custom IP** or you can just use `localhost.uptrend-data-science.com`, it is set up to point to `localhost` (to the IP `127.0.0.1`) and all its subdomains too. And as it is an actual domain, the browsers will store the cookies you set during development, etc.

If you used the default CORS enabled domains while generating the project, `localhost.uptrend-data-science.com` was configured to be allowed. If you didn't, you will need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `localhost.uptrend-data-science.com`.

After performing those steps you should be able to open: http://localhost.uptrend-data-science.com and it will be server by your stack in `localhost`.
...
If you need to use your local stack with a different domain than `localhost`, you need to make sure the domain you use points to the IP where your stack is set up. See the different ways to achieve that in the sections above (i.e. using Docker Toolbox with `local.dockertoolbox.uptrend-data-science.com`, using `localhost.uptrend-data-science.com` or using `dev.{{cookiecutter.domain_main}}`).
...
```
DOMAIN=localhost.uptrend-data-science.com
```
...
```
VUE_APP_DOMAIN_DEV=localhost.uptrend-data-science.com
```
...
You can use [`docker-auto-labels`](https://github.com/uptrend-data-science/docker-auto-labels) to automatically read the placement constraint labels in your Docker stack (Docker Compose file) and assign them to a random Docker node in your Swarm mode cluster if those labels don't exist yet.
...
Frontend: http://local.dockertoolbox.uptrend-data-science.com

Backend: http://local.dockertoolbox.uptrend-data-science.com/api/

Swagger UI: http://local.dockertoolbox.uptrend-data-science.com/swagger/

PGAdmin: http://local.dockertoolbox.uptrend-data-science.com:5050

Flower: http://local.dockertoolbox.uptrend-data-science.com:5555

Traefik UI: http://local.dockertoolbox.uptrend-data-science.com:8090
...
Frontend: http://localhost.uptrend-data-science.com

Backend: http://localhost.uptrend-data-science.com/api/

Swagger UI: http://localhost.uptrend-data-science.com/swagger/

CouchDB: http://localhost.uptrend-data-science.com:5984/_utils

Flower: http://localhost.uptrend-data-science.com:5555

Traefik UI: http://localhost.uptrend-data-science.com:8090

## Project generation and updating, or re-generating

This project was generated using https://github.com/uptrend-data-science/full-stack with:

```bash
pip install cookiecutter
cookiecutter https://github.com/uptrend-data-science/full-stack
```
...
That would be useful if, for example, the project generator (`uptrend-data-science/full-stack`) was updated and you want to integrate or review the changes.
...
```bash
cookiecutter --config-file ./cookiecutter-config-file.yml --output-dir ../project-copy https://github.com/uptrend-data-science/full-stack
```
